...
def grad_shapeSDF(x, y, z):
    if len(x) == 1:
        return single_grad_shapeSDF(np.array([x, y, z], latent_code[shape_ind], nn))
    else:
        point_batch = np.concatenate((x, y, z), 1)
        return batch_grad_shapeSDF(point_batch, latent_code[shape_ind], nn)
...
def level_set(point):
    ...
    if surface == 'torus2':
        ...
        return plane(x, y, z)
    elif surface == 'deepSDF':
        return shapeSDF(x, y, z)
    else:
        assert 0
