...
def generate_cut_elements():
    ...
    total_ids = np.asanyarray(total_ids, dtype = object)
    np.savez('data/{}_cut_element_ids.npz'.format(surface), ids=total_ids, refinement_level=total_refinement_levels, allow_pickle=True)
    return total_ids, total_refinement_levels


def main():
    data = np.load('data/{}_cut_element_ids.npz'.format(surface), allow_pickle=True)
    total_ids = data['ids']
