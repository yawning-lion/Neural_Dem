...
def plot_SDF(nn, latent_code, boundary_point, batch_seeds, shape, mode):
    step=0.01
    ...
    contour = plt.contour(X,Y,OUT,[-0.3, 0.3],colors='k')
    x_b = boundary[:, 0]
    y_b = boundary[:, 1]
    plt.scatter(x_b, y_b, s = 2, c = 'r', marker = 'o')
    seeds = batch_seeds[shape]
    x_s = seeds[:, 0]
    y_s = seeds[:, 1]
    plt.scatter(x_s, y_s, s = 0.3, c = 'b', marker = 'v')
    plt.savefig('/gpfs/share/home/1900011026/2D_deepSDF/data/img/seeds/{}_seeds{}{}.svg'.format(mode, shape // 10, shape%10), dpi = 600, format = "svg")
    plt.close()
...
def run_plot_loop(mode, index):
    ...
    batch_seeds = onp.load('/gpfs/share/home/1900011026/2D_deepSDF/data/data_set/{}_seeds.npy'.format(mode))
    for i in range(index):
        plot_SDF(nn, latent_code, boundary_point, batch_seeds, i, mode)
