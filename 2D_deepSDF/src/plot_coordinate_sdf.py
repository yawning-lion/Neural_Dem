...
x = onp.arange(-2,2,step)
y = onp.arange(-2,2,step)
...
x_b = boundary_point[6][:, 0]
y_b = boundary_point[6][:, 1]
...
x_s = onp.random.uniform(-2,2,250)
y_s = onp.random.uniform(-2,2,250)
plt.scatter(x_s, y_s, s = 10, c = 'black', marker = 'o',zorder=2)
plt.title('random samples points')
plt.xlabel('x(\mum)')
plt.ylabel('y(\mum)')
plt.axis('equal')
plt.savefig("data/img/random samples points.png", bbox_inches = "tight")
plt.close()

sample = np.stack([x_s,y_s],1)
s_2 = np.ones(len(sample)) * shape_2
s_2 = s_2.reshape(len(sample),1)
in_sample = np.concatenate([sample, s_2],1)
#print(in_sample)
in_sample = batch_append_latent(latent_code, in_sample)
sdf_sample = batch_forward(nn, in_sample)
#norm = mcolors.DivergingNorm(vmin=sdf_sample.min(), vmax = sdf_sample.max(), vcenter=0)
sc = plt.scatter(x_s,y_s,s = 10,c=sdf_sample,cmap="viridis")
#plt.scatter(x_b, y_b, s = 15,c = 'black', marker = 'o',zorder=2)
contour1 = plt.contour(X,Y,OUT_2,0, colors = 'black',linewidth = 20)
plt.title('sdf rendering of random samples points')
plt.xlabel('x(\mum)')
plt.ylabel('y(\mum)')
plt.axis('equal')
plt.colorbar(sc)
plt.savefig("data/img/sdf rendering of random samples points.png", bbox_inches = "tight")
'''